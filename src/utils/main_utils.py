...
class Mainutils:
    ...
    def save_object(file_path:str,obj:object) -> None:
        ...
        try:
            # writing trained model in binary and dumping in pickle from
    ...
    def load_object(file_path:str) -> object:
        logging.info("Entered the load_object method pr MainUtils class")

        try:
            with open(file_path,"rb") as file_obj:
                obj=pickle.load(file_obj)
            logging.info('Exited theload_object method of MainUtils class')
            return obj
        except Exception as e :
            raise CustomException(e,sys) from e
    ...
    @staticmethod
    def download_model(bucket_name,bucket_file_name,data_file_name):
        try:
            s3_client = boto3.client('s3')
            s3_client.download_file(bucket_name,bucket_file_name,data_file_name)
